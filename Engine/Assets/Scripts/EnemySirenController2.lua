...
local function PlayAnim(name, blend)
    if Mortar.anim then Mortar.anim:Play(name, blend or 0.15) end
end
...
local function TakeDamage(self, amount, attackerPos)
    ...
    if hp <= 0 then
        ...
        --if Mortar.voiceSFX then Mortar.voiceSFX:StopAudioEvent() end
        --if Mortar.waterSFX then Mortar.waterSFX:StopAudioEvent() end
...
function Start(self)
    ...
    if voiceSource then 
        ...
        if not voiceSFX then
            Engine.Log("[SIREN AUDIO] Unable to retrieve Siren Voice SFX Audio Source Component") 
        end
    else 
   ...
   waterSource = GameObject.FindInChildren(self.gameObject, "SirenWaterSource")
    if waterSource then 
        ...
        if not waterSFX then 
            Engine.Log("[SIREN AUDIO] Unable to retrieve Siren Water SFX Audio Source Component") 
        end
    else
...
function Update(self, dt)
    ...
    -- Reintentar componentes de audio si faltan
    if not voiceSFX or not waterSFX then
        FindSirenAudioComponents(self)
    end
    ...
    if playerAttack ~= "" and hideCooldownTimer <= 0 and Mortar.currentState ~= State.HIDE then
        ...
        if math.random() < currentEvadeChance then
            ...
            SelectPlaySFX(waterSFX, "SFX_SirenDip")
    ...
    if Mortar.currentState == State.HIDE then
        if  not Mortar.anim:IsPlayingAnimation("Hide") then
    ...
    elseif Mortar.currentState == State.WINDUP then
        ...
        if not Mortar.anim:IsPlayingAnimation("Charge") then
    ...
    elseif Mortar.currentState == State.COOLDOWN then
        ...
        if cooldownTimer <= 0 then
            Mortar.currentState = State.IDLE
            if not Mortar.anim:IsPlayingAnimation("Look") then
