
local atan2 = math.atan
local pi    = math.pi
local sqrt  = math.sqrt
local abs   = math.abs

local State = {
    IDLE       = "Idle",
    PATROL     = "Patrol",
    CHASE      = "Chase",
    ORBIT      = "Orbit",
    DODGE      = "Dodge",
    ANTICIPATE = "Anticipate",
    ATTACK     = "Attack",
    DEAD       = "Dead",
}
local States = {}

public = {
    maxHp               = 30,
    patrolSpeed         = 1.5,
    chaseSpeed          = 3.5,
    navRefreshRate      = 0.18,
    attackDur           = 1.0,
    attackColDelay      = 0.9,
    attackAnimaAnticip  = 0.3,
    nearDist            = 2,
    nearYDist           = 1.0,
    patrolWaitMin       = 1.0,
    patrolWaitMax       = 2.8,
}


local function Lerp(a, b, t)  return a + (b - a) * t  end

local function Clamp(v, lo, hi)
    if v < lo then return lo end
    if v > hi then return hi end
    return v
end


local function CheckDistance(self, dist, near)
    local playerPos = self.playerGO.transform.position
    local pos       = self.transform.position
    if near then
        return abs(pos.x - playerPos.x) < dist and abs(pos.z - playerPos.z) < dist
    else
        return abs(pos.x - playerPos.x) > dist or  abs(pos.z - playerPos.z) > dist
    end
end

local function ApplyMoveVelocity(self, dt, accelRate)
    local vel = self.rb:GetLinearVelocity()
    self.rb:SetLinearVelocity(
        Lerp(vel.x, self.targetVelX, dt * accelRate),
        vel.y,
        Lerp(vel.z, self.targetVelZ, dt * accelRate)
    )
end

local function FaceTargetSmooth(self, target, dt)
    local p  = self.transform.worldPosition
    local dx = target.x - p.x
    local dz = target.z - p.z
    if abs(dx) < 0.001 and abs(dz) < 0.001 then return end

    local desiredYaw = atan2(dx, dz) * (180 / pi)
    local delta      = desiredYaw - self.currentYaw
    delta = delta - math.floor((delta + 180) / 360) * 360

    local turn     = Clamp(delta, -4.0 * dt * 60, 4.0 * dt * 60)
    self.currentYaw = self.currentYaw + turn
    self.rb:SetRotation(0, self.currentYaw, 0)
end

local function FindAudioComponents(self)
    local attackSource = GameObject.FindInChildren(self.gameObject, "SK_KopisSource")
    if not attackSource then Engine.Log("Could not retrieve GameObject containing Skeleton attackSFX")
    else
        self.attackSFX = attackSource:GetComponent("Audio Source")
        if not self.attackSFX then Engine.Log("Could not retrieve Audio Source component to play Skeleton attackSFX") end
    end

    local hurtSource = GameObject.FindInChildren(self.gameObject, "SK_HurtSource")
    if not hurtSource then Engine.Log("Could not retrieve GameObject containing Skeleton hurtSFX")
    else
        self.hurtSFX = hurtSource:GetComponent("Audio Source")
        if not self.hurtSFX then Engine.Log("Could not retrieve Audio Source component to play Skeleton hurtSFX") end
    end

    local dieSource = GameObject.FindInChildren(self.gameObject, "SK_DieSource")
    if not dieSource then Engine.Log("Could not retrieve GameObject containing Skeleton deathSFX")
    else
        self.dieSFX = dieSource:GetComponent("Audio Source")
        if not self.dieSFX then Engine.Log("Could not retrieve Audio Source component to play Skeleton dieSFX") end
    end

    local dodgeSource = GameObject.FindInChildren(self.gameObject, "SK_DodgeSource")
    if not dodgeSource then Engine.Log("Could not retrieve GameObject containing Skeleton dodgeSFX")
    else
        self.dodgeSFX = dodgeSource:GetComponent("Audio Source")
        if not self.dodgeSFX then Engine.Log("Could not retrieve Audio Source component to play Skeleton dodgeSFX") end
    end

    self.stepSFX = self.gameObject:GetComponent("Audio Source")
    if not self.stepSFX then
        Engine.Log("[SKELETON AUDIO] Make sure there's an Audio Source with Steps SFX in the Skeleton's parent GameObject!")
    end
end

local function PlaySFX(audioComp)
    if audioComp then audioComp:PlayAudioEvent()
    else Engine.Log("Could not play configured event in Audio Source " .. tostring(audioComp) .. ", component not found") end
end

local function SelectPlaySFX(audioComp, eventName)
    if audioComp then audioComp:SelectPlayAudioEvent(audioComp, eventName)
    else Engine.Log("Could not play " .. eventName .. ", Audio Source component " .. tostring(audioComp) .. " not found") end
end

local function TakeDamage(self, amount, attackerPos)
    if self.isDead or not self.hp then return end

    self.hp = self.hp - amount

    _PlayerController_triggerCameraShake = true

    if self.hp <= 0 and not self.pendingDeath then
        PlaySFX(self.dieSFX)
        self.pendingDeath = true
    else
        self.hitGiven = false
        if self.nav then self.nav:StopMovement() end
        PlaySFX(self.hurtSFX)
        local anim = self.gameObject:GetComponent("Animation")
        if anim then
            -- pcall(function() anim:Play("Hit", 0.5) end)
        end
    end
end

local function ChangeState(self, newState)
    if self.currentState and States[self.currentState] and States[self.currentState].Exit then
        States[self.currentState].Exit(self)
    end
    self.currentState = newState
    if newState ~= State.IDLE and newState ~= State.RUNNING
    and newState ~= State.WALK and newState ~= State.ATTACK_LIGHT then
        self.attackBuffer = false
    end
    if States[newState] and States[newState].Enter then
        States[newState].Enter(self)
    end
end


States[State.IDLE] = {
    Enter = function(self)
        self.playerGO = GameObject.Find("Player")
        self.nav:StopMovement()
        local anim = self.gameObject:GetComponent("Animation")
        if anim then pcall(function() anim:Play("Idle", 0.5) end) end
    end,
    Update = function(self, dt)
        local px, py, pz = self.nav:GetRandomPoint()
        if px then
            self.nav:SetDestination(px, py, pz)
            ChangeState(self, State.PATROL)
        else
            self.patrolWait = self.public.patrolWaitMin
        end
        if CheckDistance(self, 30.0, true) then
            ChangeState(self, State.CHASE)
            return
        end
    end
}

States[State.PATROL] = {
    Enter = function(self)
        self.playerGO = GameObject.Find("Player")
        local anim = self.gameObject:GetComponent("Animation")
        if anim then pcall(function() anim:Play("Walk", 0.5) end) end
    end,
    Update = function(self, dt)
        local dx, dz = self.nav:GetMoveDirection(0.3)
        self.targetVelX = dx * self.public.patrolSpeed
        self.targetVelZ = dz * self.public.patrolSpeed
        ApplyMoveVelocity(self, dt, 18.0)
        if abs(dx) > 0.001 or abs(dz) > 0.001 then
            local p = self.transform.worldPosition
            FaceTargetSmooth(self, {x = p.x + dx, y = p.y, z = p.z + dz}, dt)
        end
        if not self.nav:IsMoving() then
            self.patrolWait = self.public.patrolWaitMin
                + math.random() * (self.public.patrolWaitMax - self.public.patrolWaitMin)
            ChangeState(self, State.IDLE)
            return
        end
        if CheckDistance(self, 30.0, true) then
            ChangeState(self, State.CHASE)
            return
        end
    end
}

States[State.CHASE] = {
    Enter = function(self)
        local anim = self.gameObject:GetComponent("Animation")
        if anim then pcall(function() anim:Play("Walk", 0.2) end) end
    end,
    Update = function(self, dt)
        local plPos = self.playerGO.transform.worldPosition
        if CheckDistance(self, self.public.nearDist, true) then
            ChangeState(self, State.ATTACK)
            return
        end
        if CheckDistance(self, 31, false) then
            ChangeState(self, State.IDLE)
            return
        end
        self.navRefreshTimer = self.navRefreshTimer - dt
        if self.navRefreshTimer <= 0 then
            self.nav:SetDestination(plPos.x, plPos.y, plPos.z)
            self.navRefreshTimer = self.public.navRefreshRate
        end
        local dx, dz = self.nav:GetMoveDirection(0.3)
        self.targetVelX = dx * self.public.chaseSpeed
        self.targetVelZ = dz * self.public.chaseSpeed
        ApplyMoveVelocity(self, dt, 18.0)
        FaceTargetSmooth(self, plPos, dt)
    end
}

States[State.ATTACK] = {
    Enter = function(self)
        self.nav:StopMovement()
        local anim = self.gameObject:GetComponent("Animation")
        if anim then pcall(function() anim:Play("Orbit", 0.5) end) end
    end,
    Update = function(self, dt)
        local plPos = self.playerGO.transform.worldPosition
        self.attackTimer = self.attackTimer + dt

        if self.attackTimer >= self.public.attackColDelay - self.public.attackAnimaAnticip
        and not self.hitGiven and self.playerGO then
            local pending = _PlayerController_pendingDamage or 0
            if pending == 0 then
                local anim = self.gameObject:GetComponent("Animation")
                if anim then pcall(function() anim:Play("Attack", 0.0) end) end
            end
        end

        if self.attackTimer >= self.public.attackColDelay
        and not self.hitGiven and self.playerGO then
            local pending = _PlayerController_pendingDamage or 0
            if pending == 0 then
                self.hitGiven = true
                PlaySFX(self.attackSFX)
                _PlayerController_pendingDamage    = 20
                _PlayerController_pendingDamagePos = self.transform.worldPosition
            end
        end

        if self.attackTimer >= self.public.attackDur or self.alreadyHit then
            local anim = self.gameObject:GetComponent("Animation")
            if anim then pcall(function() anim:Play("Orbit", 0.5) end) end
            self.hitGiven   = false
            self.attackTimer = 0
        end

        if CheckDistance(self, self.public.nearDist, false) then
            ChangeState(self, State.CHASE)
            self.hitGiven   = false
            self.attackTimer = 0
            return
        end
        FaceTargetSmooth(self, plPos, dt)
        self.rb:SetLinearVelocity(0, 0, 0)
    end
}


function Init(self)
    

    self.hp              = self.public.maxHp
    self.isDead          = false
    self.pendingDeath    = false
    self.alreadyHit      = false
    self.attackTimer     = 0
    self.hitGiven        = false
    self.patrolWait      = 0
    self.navRefreshTimer = 0
    self.targetVelX      = 0
    self.targetVelZ      = 0
    self.stepTimer       = 0.5
    self.attackBuffer    = false
    self.currentState    = nil
    self.currentYaw      = (self.transform.worldRotation and self.transform.worldRotation.y) or 0

    self.playerGO = GameObject.Find("Player")
    self.rb       = self.gameObject:GetComponent("Rigidbody")
    if not self.rb  then Engine.Log("[Skeleton] No Rigidbody found") end

    self.nav = self.gameObject:GetComponent("Navigation")
    if not self.nav then Engine.Log("[Skeleton] No Navigation found") end

    FindAudioComponents(self)

    local skeletonMesh = GameObject.FindInChildren(self.gameObject, "Mesh_fixedUVs")
    self.BaseMat = skeletonMesh:GetComponent("Material")

    ChangeState(self, State.IDLE)
    self.initialized = true
end

local function Start(self)
    Init(self)
end

function Update(self, dt)

    if not self.initialized then
        Init(self)
        return
    end

    if not self.nav then
        self.nav = self.gameObject:GetComponent("Navigation")
    end
    if not self.rb then
        self.rb = self.gameObject:GetComponent("Rigidbody")
    end
    if not self.currentState then
        ChangeState(self, State.IDLE)
    end
    if self.currentState and States[self.currentState] then
        States[self.currentState].Update(self, dt)
    end

    if self.pendingDeath then
        if self.nav then self.nav:StopMovement() end
        self.isDead = true
        self:Destroy()
        return
    end

    
    if self.currentState == State.PATROL or self.currentState == State.CHASE
    or self.currentState == State.ORBIT  or self.currentState == State.DODGE then
        self.stepTimer = self.stepTimer + dt
        if self.stepTimer >= 0.5 then
            self.stepTimer = 0
            PlaySFX(self.stepSFX)
        end
    else
        self.stepTimer = 0
    end
end

function OnTriggerEnter(self, other)
    if self.isDead then return end
    if other:CompareTag("Player") then
        if not self.alreadyHit then
            local attack = _PlayerController_lastAttack
            if attack ~= nil and attack ~= "" then
                local ap  = other.transform.worldPosition
                local dmg = 0
                if     attack == "light"                       then dmg = 10
                elseif attack == "heavy" or attack == "charge" then dmg = 25
                end
                if dmg > 0 then
                    self.alreadyHit = true
                    self.BaseMat.SetTexture("17109277834976977864")
                    TakeDamage(self, dmg, ap)
                end
            end
        end
    end
end

function OnTriggerExit(self, other)
    if other:CompareTag("Player") then
        self.alreadyHit = false
        self.BaseMat.SetTexture("13296577326446124640")
    end
end